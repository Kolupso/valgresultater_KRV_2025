...
if sfu_valg == "Nej":
    ...
    query = "SELECT kandidat as Kandidat, antal_stemmer as `Antal stemmer` FROM workspace.valgresultat.personlige_stemmer_krv_2025 WHERE SFU_kandidat = 'Ja' ORDER BY kandidat"
    with sql.connect(server_hostname=SERVER_HOST, http_path=HTTP_PATH, access_token=ACCESS_TOKEN) as c:
        with c.cursor() as cur:
            cur.execute(f"{query}")
            df = pd.DataFrame(cur.fetchall(), columns=[d[0] for d in cur.description])
